...
abstract sig Ticket {
	...
	// A ticket must be used after it has been issued
...
sig BookedTicket extends Ticket {
	...
	// A ticket must be booked before the timeslot starts
	precedesStamp[issued, timeSlot.start]
	// A ticket must be used within its timeslot
...
sig TimeSlot {
    ...
    // The timeslot end timestamp is greater than the slot's start timestamp
...
fact timeSlotNotOverlapping {
	// A previous timeslot's start and end timestamps are before the current timeslot, 
	// there is no overlapping of timeslots
...
fact firstComeFirstServed {
	...
	// A customer enters the store only if all people before him have entered 
	// the store or no-showed
