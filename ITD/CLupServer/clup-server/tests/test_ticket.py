...
class ClientInfo:
    ...
    def __init__(self, token, ticket_id, status):
        self.token = token
...
class TestTicket:
    ...
    def test_call_empty(self, flask_client, populate):
        operator, _ = login(flask_client, "operator3@CLup.com", "operator3@CLup.com")

        response = post_json(
            flask_client, "/call_first",{}, bearer=operator
        )
        assert response.status_code == 200
        assert response.json["success"]
        assert response.json["queue_empty"]
    ...
    def test_double_create(self, flask_client, populate):
        ...
        response = post_json(
            flask_client, "/create_ticket", {"store_id": 2}, bearer=auth
        )
        assert response.status_code == 200
        assert response.json["success"]
        ...
        response = post_json(
            flask_client, "/create_ticket", {"store_id": 3}, bearer=auth
        )
        assert response.status_code == 200
        assert not response.json["success"]
        assert "ticket" not in response.json
        ...
        # Get Ticket Info
        response = post_json(
            flask_client, "/ticket_info", {"ticket_id": ticket_id}, bearer=operator
        )
        assert response.status_code == 200
        assert response.json["success"]
        assert "ticket" in response.json
    ...
    def test_queue_evolution(self, flask_client, populate):
        ...
        operator_token = login(
            flask_client, f"operator1@CLup.com", f"operator1@CLup.com"
        )[0]
        # LogIn All
        clients = OrderedDict()
        ...
        for i in range(1, N_TICKETS):
            token = login(
                flask_client, f"customer{i}@CLup.com", f"customer{i}@CLup.com"
            )[0]
        ...
        for i in range(1, N_TICKETS):
            response = post_json(
                flask_client, "/create_ticket", {"store_id": 1}, bearer=clients[i].token
            )
        ...
        for i in range(1, 11, 2):
            # Cancel ticket
            response = post_json(
                flask_client, "/cancel_ticket", {}, bearer=clients[i].token
            )
            ...
            response = post_json(
                flask_client, "/active_ticket", {}, bearer=clients[i].token
            )
        ...
        for i, info in clients.items():
            if info.status == Ticket.STATE_ISSUED:
                response = post_json(
                    flask_client, "/active_ticket", {}, bearer=info.token
                )
                assert response.status_code == 200
                assert response.json["success"]
                ticket = response.json["ticket"]
        ...
        called = [
            i for i, info in clients.items() if info.status == Ticket.STATE_ISSUED
        ][:N_CALLS]
        for i in called:
        ...
        for i, info in clients.items():
            if info.status == Ticket.STATE_CALLED:
                ...
                response = post_json(
                    flask_client, "/store_info", {"store_id": 1}, bearer=operator_token
                )
                assert response.status_code == 200
                assert response.json["success"]
        ...
        for real_time_capacity in range(N_CALLS - 1, -1, -1):
            response = post_json(
                flask_client, "/count_store_leave", {}, bearer=operator_token
            )
            assert response.status_code == 200
            assert response.json["success"]
