...
class TestAuthentication:
    ...
    def test_register(self, populate, flask_client):
        reg_payload = dict(
            name="John Doe", email="John@Doe.com", password="@1Aasfjakdjjd"
        )
        response = post_json(flask_client, "/register", reg_payload)
        print(response.json)
        assert response.status_code == 200
        ...
        # Not Valid Email
        reg_payload = dict(
            name="John Doe", email="JohnDoe.com", password="@1Aasfjakdjjd"
        )
        response = post_json(flask_client, "/register", reg_payload)
        print(response.json)
        assert response.status_code == 200
        ...
        # Password
        reg_payload = dict(
            name="John Doe", email="John2@Doe.com", password="@1234provae"
        )
        response = post_json(flask_client, "/register", reg_payload)
        print(response.json)
        assert response.status_code == 200
    ...
    def test_login(self, flask_client, account):
        # Wrong Password test
        wrong_password = dict(
            email=account["email"], password=account["password"] + "e"
        )
