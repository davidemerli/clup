...
def expired_token_callback(expired_token):
    ...
    return (
        jsonify(
            {
                "success": False,
                "errors": {"auth": f"The {token_type} token has expired"},
            }
        ),
        401,
    )
...
def invalid_token_callback(invalid_token):
    return jsonify({"success": False, "errors": {"auth": f"The token is invalid"}}), 401
...
def unauthorized_callback(invalid_token):
    return (
        jsonify(
            {
                "success": False,
                "errors": {"auth": f"The authorization token is missing"},
            }
        ),
        401,
    )
